...
wxString wxStyledTextCtrl::StyleGetFont(int style) {
     int len = SendMsg(2486, style, (long)NULL);
     if (!len) return wxEmptyString;

     wxMemoryBuffer mbuf(len+1);
     char* buf = (char*)mbuf.GetWriteBuf(len+1);
     SendMsg(2486, style, (long)buf);
     mbuf.UngetWriteBuf(len);
     mbuf.AppendByte(0);
     return stc2wx(buf);
}
...
void wxStyledTextCtrl::SetPositionCache(int size) {
    SendMsg(2514, size, 0);
}

// How many entries are allocated to the position cache?
int wxStyledTextCtrl::GetPositionCache() {
